...
import { 
  Send, Users, Clock, Trash2, Edit, X, Loader2
} from 'lucide-react';
...
export default function UpdatesPage() {
  ...
  const onSubmit = async (data: UpdateFormValues) => {
    ...
    try {
      ...
      const titleWithPrefix = data.type !== 'update' 
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>{editingId ? 'Modify Update' : 'New Update'}</CardTitle>
              <CardDescription>Compose a message to your class or a specific student's parents.</CardDescription>
            </div>
            {editingId && (
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Science Fair Project Due" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className="text-xs text-muted-foreground border rounded-md p-3 bg-muted/20">
                  Note: Custom parent updates are pushed as priority notifications to the ConnecTED mobile app.
                </div>

                <Button type="submit" className="w-full" disabled={isLoading}>
                  {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                  {isLoading ? (editingId ? 'Saving Changes...' : 'Sending Update...') : (editingId ? 'Save Changes' : 'Send Update')}
            ...
            myUpdates.map((update) => (
              <Card key={update.id || update._id}>
                ...
                <CardFooter className="pt-0 flex justify-end gap-2">
                   ...
                   <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => handleDelete(update.id || update._id)}>
