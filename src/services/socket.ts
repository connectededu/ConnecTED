...
// Typing events
export const onTypingStart = (callback: (data: { userId: string; threadId: string }) => void): void => {
  on('typing:start', callback);
};

export const onTypingStop = (callback: (data: { userId: string; threadId: string }) => void): void => {
